...
import TavusPlayer from '@/components/tavus-player';
...
const LessonViewer = ({ lesson }: LessonViewerProps) => {
   ...
   return lesson ? (
      <Card className={cn('group lesson-container relative')}>
         ...
         {lesson.lesson_type === 'tavus' && (
            <TavusPlayer
               conversationId={lesson.tavus_conversation_id}
               replicaId={lesson.tavus_replica_id}
               apiKey={lesson.tavus_api_key}
               translate={translate}
            />
         )}
